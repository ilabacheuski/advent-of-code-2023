...
pub struct Constraints(usize, usize, usize);

pub fn sum_of_possible_games(input: &str, constraints: Constraints) -> usize {
    let mut m = HashMap::new();
    m.insert("red", constraints.0);
    m.insert("green", constraints.1);
    m.insert("blue", constraints.2);

    input
        .lines()
        .map(|line| {
            let (game, data) = line.split_once(':').unwrap();
            let (_, game_number_str) = game.split_once(' ').unwrap();
            let game_number = game_number_str.parse::<usize>().unwrap();
            ...
            let is_valid = data
                ...
                .map(|iteration| {
                    iteration
                        .split(',')
                        .map(|cube_str| {
                            let (count, color) = cube_str.trim().split_once(' ').unwrap();
                            let count = count.parse::<usize>().unwrap();
                            count <= *m.get(color).unwrap()
                        })
                        .all(|x| x)
                })
                .all(|x| x);
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
        ...
        assert_eq!(8, sum_of_possible_games(input, Constraints(12, 13, 14)));
    }

    #[test]
    fn test_d2_2_2() {
        let input = "Game 1: 1 green, 2 blue; 15 blue, 12 red, 2 green; 4 red, 6 blue; 10 blue, 8 red; 3 red, 12 blue; 1 green, 12 red, 8 blue
...
Game 100: 1 blue, 1 red, 1 green; 8 blue, 1 green; 1 green, 7 blue, 1 red; 1 green, 4 blue, 1 red; 1 green, 3 blue";
        assert_eq!(2913, sum_of_possible_games(input, Constraints(12, 13, 14)));
    }
}
