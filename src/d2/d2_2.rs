...
pub fn sum_of_possible_games2(input: &str) -> usize {
    input
        .lines()
        .map(|line| {
            ...
            data.split(';').for_each(|iteration| {
                iteration.split(',').for_each(|cube_str| {
                    let (count, color) = cube_str.trim().split_once(' ').unwrap();
                    let count = count.parse::<usize>().unwrap();
                    let min_now = min.get_mut(color).unwrap();
                    if count > *min_now {
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
        ...
        assert_eq!(2286, sum_of_possible_games2(input));
    }
...
Game 100: 1 blue, 1 red, 1 green; 8 blue, 1 green; 1 green, 7 blue, 1 red; 1 green, 4 blue, 1 red; 1 green, 3 blue";
        assert_eq!(55593, sum_of_possible_games2(input));
    }
}
